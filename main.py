...
def share_result(title, lines):
    ...
    lines: list of strings, e.g. ["Average Profit: 0.1234", "Win Rate: 77.90%"]
    """
    separator = "─" * 32
...
def log_history_entry(amount):
    """Append one profit/loss entry to history. Called on every UPDATE."""
...
def show_onboarding_wizard(on_complete=None):
    ...
    STEPS = [
        {
            "title": "Step 1 of 3 — Bankroll Size",
            "question": "What's your typical playing bankroll?",
            "key": "bankroll",
            "options": [
                ("Micro",  "Under $10",   "#95a5a6"),
                ("Small",  "$10 – $50",   DICE_COLOR),
                ("Medium", "$50 – $200",  LIMBO_COLOR),
                ("Large",  "$200+",       STAKE_GREEN),
            ],
        },
        {
            ...
            "options": [
                ...
                ("Aggressive",   "High risk, high reward", MINES_COLOR),
                ("Degen",        "All in, no fear",        STAKE_RED),
            ],
        },
        {
            ...
            "key": "game",
            "options": [
                ("Dice",   "Dice & Limbo",   DICE_COLOR),
                ("Keno",   "Keno",           KENO_COLOR),
                ("Mines",  "Mines",          MINES_COLOR),
                ("Sports", "Sports Betting", SPORTS_COLOR),
            ],
        },
    ]
...
MENU_TILE_LABELS = {
    ...
    "dice_gen": "Dice Auto\nGenerator",
    "forge": "Strategy Forge",
    "dice_evo": "Dice Evolution",
    "limbo_evo": "Limbo Evolution",
    "keno_evo": "Keno Evolution",
    "mines_evo": "Mines Evolution",
    "stress_lab": "Stress Test",
    "survival_lab": "Bankroll Survival",
    "keno_mc": "Keno Monte Carlo",
    "mines": "Mines Analytics",
    "bj": "Blackjack Hub",
    "sports_lab": "Sports Betting Lab",
    "sports_kelly": "Kelly Calculator",
    "sports_parlay": "Parlay Analyzer",
    "sports_value": "Value Bet Calc",
    "sports_arb": "Arbitrage Calc",
...
def get_display_unit_prefix():
    return 'U' if is_presentation_mode_active() else '$'


def get_display_app_name():
    return PRESENTATION_APP_NAME if is_presentation_mode_active() else APP_NAME


def get_display_subtitle():
    return PRESENTATION_SUBTITLE if is_presentation_mode_active() else "Private gambling analytics toolkit"
...
def get_tool_display_title(sid):
    ...
    return TOOL_TITLES.get(sid, sid)
...
MONTE_CARLO_HELP = """Monte Carlo simulates the same betting setup across many sessions so you can see how it behaves over time.

Recommended defaults:
• Number of Sessions: 5,000 for balanced testing
• Max Bets / Session: 100 for standard testing, 200 for deeper testing
• Increase on Loss % should match the progression you plan to test
• Stop Profit / Stop Loss can stay at 0 if you want an uncapped session

Multiplier and Win Chance are linked using the app's 99 / multiplier rule, so changing the multiplier auto-fills win chance."""
...
• Max Bets / Session: session length cap
• Top Results: how many winners are shown at the end

Balanced starting point: Population 24, Generations 6, Elite Keep 6, Children 24, Sessions 400."""

DICE_EVO_HELP = """Dice Evolution Engine mutates and ranks Dice progressions across generations.

Use it when you want the app to evolve better Dice setups automatically.

Key fields:
• Win Chance %: base hit chance used for all simulated strategies
...
• Max Bets / Session: practical cap for each simulation
...
LIMBO_EVO_HELP = """Limbo Evolution Engine evolves Limbo target-multiplier strategies across generations.

It works like Dice Evolution, but the winning chance is derived from each target multiplier.

Use higher Sessions / Strategy for more stable rankings, and increase Max Bets / Session for long recovery systems."""

KENO_EVO_HELP = """Keno Evolution Engine evolves Keno setups using tiles, target hits, progression and selection pressure.

Population, Generations, Elite Keep and Children Per Generation control how broad and how deep the search becomes.
Sessions / Strategy controls reliability, while Max Bets / Session caps how long each simulated run can recover."""

MINES_EVO_HELP = """Mines Evolution Engine evolves Mines setups using mines count, safe picks and progression logic.

Use it to search for high-quality Mines combinations automatically, then save the strongest results to the strategy library.

Higher Sessions / Strategy improves confidence; higher Max Bets / Session allows deeper recovery paths."""
...
class StrategyLibraryScreen(Screen):
    ...
    def _show_comparison_popup(self, sa, sb, ra, rb, sessions):
        ...
        # Metrics rows
        metrics = [
            ("Avg Profit",    f"{ra['average_profit']:.4f}",  f"{rb['average_profit']:.4f}",  ra['average_profit'],  rb['average_profit'],  True),
            ("Median Profit", f"{ra['median_profit']:.4f}",   f"{rb['median_profit']:.4f}",   ra['median_profit'],   rb['median_profit'],   True),
            ("Win Rate",      f"{ra['win_rate']:.2f}%",       f"{rb['win_rate']:.2f}%",       ra['win_rate'],        rb['win_rate'],        True),
            ("Bust Rate",     f"{ra['bust_rate']:.2f}%",      f"{rb['bust_rate']:.2f}%",      ra['bust_rate'],       rb['bust_rate'],       False),
            ...
            ("Avg Bets",      f"{ra['avg_bets']:.1f}",        f"{rb['avg_bets']:.1f}",        None,                  None,                  True),
        ]
        ...
        def do_share(*args):
            lines = [
                ...
                f"{'─'*18} {'─'*12} {'─'*12}",
                f"{'Avg Profit':<18} {ra['average_profit']:>12.4f} {rb['average_profit']:>12.4f}",
                f"{'Median Profit':<18} {ra['median_profit']:>12.4f} {rb['median_profit']:>12.4f}",
                f"{'Win Rate':<18} {ra['win_rate']:>11.2f}% {rb['win_rate']:>11.2f}%",
                f"{'Bust Rate':<18} {ra['bust_rate']:>11.2f}% {rb['bust_rate']:>11.2f}%",
...
"""
Casino Tools Pro — Dice Simulator v2
...
# ── Dice Roll Graphic (Stake.com style slider bar) ────────────────────────────

class DiceRollGraphic(Widget):
    """
    Visual slider bar showing:
    - Dark track background
    - Coloured win zone (green=win, blue=neutral, red=loss)
    ...
    def redraw(self, *_):
        ...
        if self.last_roll is not None:
            ...
            roll_col = _GREEN if self.win else (_RED if self.win is False else _TEXT)
...
class DiceHistoryGraph(Widget):
    """
    Live balance graph inspired by Stake.com:
    - Dark background
...
class DiceSimScreen(Screen):
    ...
    def on_pre_enter(self, *args):
        ...
        try:
            if is_presentation_mode_active():
                ...
                self.title_lbl.text = 'Dice Simulator  [size=11][color=#9a9a9a]up to 200x[/color][/size]'
    ...
    def _build_ui(self):
        ...
        self.title_lbl = title_lbl = Label(
            text='Dice Simulator  [size=11][color=#9a9a9a]up to 200x[/color][/size]',
        ...
        self._bal_card = DiceStatCard('Balance', accent=_GREEN, value_font='17sp', title_font='10sp', title_height=dp(14), card_padding=dp(10), size_hint_x=1.15)
        self._bet_card = DiceStatCard('Bet', accent=_BLUE, value_font='17sp', title_font='10sp', title_height=dp(14), card_padding=dp(10), size_hint_x=1.15)
        self._pnl_card = DiceStatCard('P / L', accent=_YELLOW, value_font='15sp', title_font='10sp', title_height=dp(14), card_padding=dp(10), size_hint_x=0.9)
        self._wr_card  = DiceStatCard('Win Rate', accent=_PURPLE, value_font='15sp', title_font='10sp', title_height=dp(14), card_padding=dp(10), size_hint_x=0.9)
        ...
        self.base_bet_in.hint_text = 'Base Bet'
        self.base_bet_in.height = dp(34)
        inputs_row.add_widget(self._labelled('Capital',  self.capital_in))
        inputs_row.add_widget(self._labelled('Base Bet', self.base_bet_in))
        ...
        self._target_lbl = Label(text='Roll UNDER', color=_hex(_TEXT),    font_size='10sp', halign='left')
        ...
        self._under_btn = _RoundedButton(text='UNDER', bg=_GREEN)
        self._under_btn.font_size = '12sp'
        self._over_btn  = _RoundedButton(text='OVER',  bg=_UTILITY)
        ...
        win_lbl = Label(text='Win:', color=_hex(_SUBTEXT), font_size='10sp',
        ...
        loss_lbl = Label(text='Loss:', color=_hex(_SUBTEXT), font_size='10sp',
                         size_hint_x=None, width=dp(28), halign='left', valign='middle')
        ...
        self._stop_profit_in.hint_text = 'Stop Profit'
        self._stop_profit_in.height = dp(34)
        self._stop_loss_in = _StyledInput(text='200')
        self._stop_loss_in.hint_text = 'Stop Loss'
        self._stop_loss_in.height = dp(34)
        self._max_bets_in = _StyledInput(text='20')
        self._max_bets_in.hint_text = 'Max Bets'
        self._max_bets_in.height = dp(34)
        self._min_bal_in = _StyledInput(text='700')
        self._min_bal_in.hint_text = 'Min Bal'
        self._min_bal_in.height = dp(34)
        stop_grid.add_widget(self._labelled('Stop Profit', self._stop_profit_in))
        stop_grid.add_widget(self._labelled('Stop Loss',   self._stop_loss_in))
        left.add_widget(stop_grid)

        stop_grid2 = GridLayout(cols=2, spacing=dp(4), size_hint_y=None, height=dp(52))
        stop_grid2.add_widget(self._labelled('Max Bets', self._max_bets_in))
        stop_grid2.add_widget(self._labelled('Min Bal',  self._min_bal_in))
        ...
        self._roll_btn = _RoundedButton(text='ROLL NOW', bg=_GREEN)
        ...
        self._wag_card   = DiceStatCard('Wagered', accent=_TEAL, value_font='15sp', title_font='10sp', title_height=dp(13), card_padding=dp(8))
        self._bets_card  = DiceStatCard('Bets', accent=_SUBTEXT, value_font='15sp', title_font='10sp', title_height=dp(13), card_padding=dp(8))
        self._wstrk_card = DiceStatCard('W.Streak', accent=_GREEN, value_font='15sp', title_font='10sp', title_height=dp(13), card_padding=dp(8))
        self._lstrk_card = DiceStatCard('L.Streak', accent=_RED, value_font='15sp', title_font='10sp', title_height=dp(13), card_padding=dp(8))
        ...
        graph_lbl = Label(
            text='Balance History', color=_hex(_TEXT), font_size='11sp', bold=True,
        ...
        log_hdr = Label(
            text='Recent Rolls', color=_hex(_TEXT), font_size='11sp', bold=True,
    ...
    def _sync_mult(self, from_slider=False):
        ...
        mode_txt = f'Roll OVER {100.0 - chance:.2f}' if is_over else f'Roll UNDER {chance:.2f}'
    ...
    def execute_roll(self):
        ...
        if bet <= 0:
            self._write_log('Bet must be > 0.')
            return 'stop'
        if bet > s.balance:
            self._write_log('Stopped: insufficient balance.')
            return 'stop'
        ...
        result_word = 'WIN' if win else 'LOSS'
        mode_word   = 'OVER' if is_over else 'UNDER'
        self._write_log(
            f'#{s.total_bets} | {roll:.4f} | {mode_word} | {result_word} | '
            f'Bet {bet:.6f} | Bal {s.balance:.6f}'
        )
    ...
    def _check_stops(self):
        ...
        if stop_prof > 0 and s.profit >= stop_prof:
            self._write_log(f'Stopped: profit target {stop_prof} reached.')
            return False
        if stop_loss > 0 and s.profit <= -stop_loss:
            self._write_log(f'Stopped: loss limit {stop_loss} reached.')
            return False
        if max_bets > 0 and s.total_bets >= max_bets:
            self._write_log(f'Stopped: max bets {max_bets} reached.')
            return False
        if min_bal > 0 and s.balance <= min_bal:
            self._write_log(f'Stopped: balance fell below {min_bal}.')
            return False
        if s.balance <= 0:
            self._write_log('Stopped: balance exhausted.')
            return False
        return True
    ...
    def _start_auto(self, *_):
        ...
        self._set_mode('auto')
        self._write_log('Auto started.')
        self._auto_step()

    def _stop_auto(self, *_):
        if self.is_auto_running:
            self._write_log('Auto stopped.')
        self.is_auto_running = False
        if self._auto_event:
            try:
    ...
    def _refresh_stats(self):
        ...
        self._graph_info.text = (
            f'Hi {s.max_balance:.4f}  '
            f'Lo {s.min_balance:.4f}  '
...
class DiceScreen(Screen):
    def __init__(self, **kw):
        ...
        layout.add_widget(build_info_header("DICE / LIMBO CALCULATOR"))
        self.inputs = {}
    ...
    def clear_values(self, *args):
        ...
        self.stats_lbl.text = "Streak Analysis: --"
...
class MonteCarloScreen(Screen):
    def __init__(self, **kw):
        ...
        self.mc_inputs["Multiplier"].bind(text=self.on_multiplier_change)
        ...
        fill_btn = StyledButton(text="LOAD FROM\nDICE CALC", bg_color="#2c3e50", height=dp(52))
        ...
        self.on_multiplier_change(None, self.mc_inputs["Multiplier"].text)
    ...
    def on_multiplier_change(self, instance, value):
        ...
        try:
            ...
            self.mc_inputs["Win Chance %"].text = f"{chance:.2f}"
        except Exception:
            pass
        finally:
            self._mc_sync_guard = False
    ...
    def _share_mc_result(self, *args):
        ...
        share_result("Monte Carlo Simulator", [
            ...
            f"Average Profit:        {r['average_profit']:.4f}",
            f"Median Profit:         {r['median_profit']:.4f}",
            f"Best Session:          {r['best_session']:.4f}",
            f"Worst Session:         {r['worst_session']:.4f}",
            f"Win Rate:              {r['win_rate']:.2f}%",
            f"Bust Rate:             {r['bust_rate']:.2f}%",
...
class KenoMonteCarloScreen(Screen):
    def __init__(self, **kw):
        ...
        outer.add_widget(build_info_header("KENO MONTE CARLO"))
...
class DiceAutoGeneratorScreen(Screen):
    def __init__(self, **kw):
        ...
        outer.add_widget(build_info_header("DICE AUTO GENERATOR"))
...
class DiceEvolutionScreen(Screen):                                                                                 
    def __init__(self, **kw):
        ...
        outer.add_widget(build_info_header("DICE EVOLUTION ENGINE", DICE_EVO_HELP))
...
class LimboEvolutionScreen(Screen):                                                                                              
    def __init__(self, **kw):
        ...
        outer.add_widget(build_info_header("LIMBO EVOLUTION ENGINE", LIMBO_EVO_HELP))
    ...
    def clear_values(self, *args):
        ...
        self.progress_bar.value = 0
        self.status_lbl.text = "Status: Idle"
        self.summary.text = "Limbo evolution results will appear here"
...
class KenoEvolutionScreen(Screen):
    def __init__(self, **kw):
        ...
        outer.add_widget(build_info_header("KENO EVOLUTION ENGINE", KENO_EVO_HELP))
    ...
    def clear_values(self, *args):
        ...
        self.progress_bar.value = 0
        self.status_lbl.text = "Status: Idle"
        self.summary.text = "Keno evolution results will appear here"
...
class MinesEvolutionScreen(Screen):
    def __init__(self, **kw):
        ...
        outer.add_widget(build_info_header("MINES EVOLUTION ENGINE", MINES_EVO_HELP))
    ...
    def clear_values(self, *args):
        ...
        self.progress_bar.value = 0
        self.status_lbl.text = "Status: Idle"
        self.summary.text = "Mines evolution results will appear here"
...
class SportsKellyScreen(Screen):
    def __init__(self, **kw):
        ...
        layout.add_widget(build_info_header("KELLY BET CALCULATOR"))
    ...
    def calculate(self, *args):
        ...
        try:
            ...
            # Share button
            self._kelly_last = {
                'bankroll': bankroll, 'odds': odds, 'est_win_pct': est_win_pct,
                'implied_prob': implied_prob, 'edge_pct': edge_pct,
                'kelly_fraction': kelly_fraction, 'full_kelly_amt': full_kelly_amt,
                'half_kelly_amt': half_kelly_amt, 'quarter_kelly_amt': quarter_kelly_amt,
                'expected_value_per_unit': expected_value_per_unit,
                'expected_profit_on_full': expected_profit_on_full,
            }
            share_btn = StyledButton(text="SHARE RESULT", bg_color=UTILITY_COLOR)
            share_btn.color = (1, 1, 1, 1)
            share_btn.size_hint_y = None
    ...
    def _share_kelly_result(self, *args):
        ...
        if not r:
            return
        share_result("Kelly Bet Calculator", [
            f"Bankroll:              {r['bankroll']:.2f}",
            f"Decimal Odds:          {r['odds']:.2f}",
            f"Estimated Win:         {r['est_win_pct']:.2f}%",
            f"Implied Probability:   {r['implied_prob']:.2f}%",
            f"Your Edge:             {r['edge_pct']:.2f}%",
            f"Full Kelly:            {r['kelly_fraction']*100:.2f}% of bankroll",
            f"Full Kelly Stake:      {r['full_kelly_amt']:.4f}",
            f"Half Kelly Stake:      {r['half_kelly_amt']:.4f}",
            f"Quarter Kelly Stake:   {r['quarter_kelly_amt']:.4f}",
            f"EV / Unit Staked:      {r['expected_value_per_unit']:.4f}",
            f"Expected Profit:       {r['expected_profit_on_full']:.4f}",
        ])
...
class SportsParlayScreen(Screen):
    def __init__(self, **kw):
        ...
        layout.add_widget(build_info_header("PARLAY ANALYZER"))
    ...
    def calculate(self, *args):
        ...
        try:
            ...
            # Share button
            self._parlay_last = {
                'combined_odds': combined_odds, 'true_parlay_prob': true_parlay_prob,
                'implied_parlay_prob': implied_parlay_prob, 'edge_pct': edge_pct,
                'payout': payout, 'net_profit_if_win': net_profit_if_win,
                'expected_value': expected_value, 'roi_pct': roi_pct,
                'bust_prob': bust_prob, 'stake': stake,
            }
            share_btn = StyledButton(text="SHARE RESULT", bg_color=UTILITY_COLOR)
            share_btn.color = (1, 1, 1, 1)
            share_btn.size_hint_y = None
            share_btn.height = dp(42)
            share_btn.bind(on_release=self._share_parlay_result)
            self.result_box.add_widget(share_btn)
    ...
    def _share_parlay_result(self, *args):
        r = getattr(self, '_parlay_last', None)
        if not r:
            return
        share_result("Parlay Analyzer", [
            f"Stake:                    {r['stake']:.2f}",
            f"Combined Odds:            {r['combined_odds']:.4f}",
            f"True Win Probability:     {r['true_parlay_prob']*100:.2f}%",
            f"Implied Probability:      {r['implied_parlay_prob']*100:.2f}%",
            f"Edge:                     {r['edge_pct']:.2f}%",
            f"Payout if Win:            {r['payout']:.4f}",
            f"Net Profit if Win:        {r['net_profit_if_win']:.4f}",
            f"Expected Value:           {r['expected_value']:.4f}",
            f"Expected ROI:             {r['roi_pct']:.2f}%",
            f"Bust Probability:         {r['bust_prob']:.2f}%",
        ])
...
class SportsValueBetScreen(Screen):
    def __init__(self, **kw):
        ...
        layout.add_widget(build_info_header("VALUE BET CALCULATOR"))
    ...
    def calculate(self, *args):
        ...
        try:
            ...
            # Share button
            self._value_last = {
                'stake': stake, 'odds': odds, 'est_win_pct': est_win_pct,
                'implied_pct': implied_pct, 'fair_odds': fair_odds,
                'edge_pct': edge_pct, 'payout': payout,
    ...
    def _share_value_result(self, *args):
        ...
        if not r:
            return
        share_result("Value Bet Calculator", [
            f"Stake:                    {r['stake']:.2f}",
            f"Decimal Odds:             {r['odds']:.2f}",
            f"Estimated Win:            {r['est_win_pct']:.2f}%",
            f"Implied Probability:      {r['implied_pct']:.2f}%",
            f"Fair Odds:                {r['fair_odds']:.4f}",
            f"Edge:                     {r['edge_pct']:.2f}%",
            f"Payout if Win:            {r['payout']:.4f}",
            f"Net Profit if Win:        {r['net_profit_if_win']:.4f}",
            f"Expected Value:           {r['expected_value']:.4f}",
...
class SportsArbitrageScreen(Screen):
    def __init__(self, **kw):
        ...
        layout.add_widget(build_info_header("ARBITRAGE CALCULATOR"))
    ...
    def calculate(self, *args):
        ...
        try:
            ...
            # Share button
            self._arb_last = {
                'total_stake': total_stake, 'odds_a': odds_a, 'odds_b': odds_b,
                'inv_sum': inv_sum, 'stake_a': stake_a, 'stake_b': stake_b,
                'payout_a': payout_a, 'payout_b': payout_b,
                'locked_payout': locked_payout,
                'guaranteed_profit': guaranteed_profit,
                'roi_pct': roi_pct, 'verdict': verdict,
            }
            share_btn = StyledButton(text="SHARE RESULT", bg_color=UTILITY_COLOR)
            share_btn.color = (1, 1, 1, 1)
            share_btn.size_hint_y = None
    ...
    def _share_arb_result(self, *args):
        ...
        if not r:
            return
        share_result("Arbitrage Calculator", [
            f"Total Stake:           {r['total_stake']:.2f}",
            f"Outcome A Odds:        {r['odds_a']:.2f}",
            f"Outcome B Odds:        {r['odds_b']:.2f}",
            f"Inverse Sum:           {r['inv_sum']:.6f}",
            f"Outcome A Stake:       {r['stake_a']:.4f}",
            f"Outcome B Stake:       {r['stake_b']:.4f}",
            f"Outcome A Payout:      {r['payout_a']:.4f}",
            f"Outcome B Payout:      {r['payout_b']:.4f}",
            f"Locked Payout:         {r['locked_payout']:.4f}",
            f"Guaranteed Profit:     {r['guaranteed_profit']:.4f}",
            f"Guaranteed ROI:        {r['roi_pct']:.2f}%",
            f"Verdict:               {r['verdict']}",
        ])
...
class StrategyStressTestScreen(Screen):
    ...
    def _on_multiplier_target_change(self, instance, value):
        ...
        try:
            ...
            self.inputs["Win Chance %"].text = f"{chance:.2f}"
        except Exception:
            pass
        finally:
            self._stress_sync_guard = False
    ...
    def _on_game_change(self, instance, value):
        ...
        self._set_field_enabled("Target Hits", is_keno)
        self._set_field_enabled("Mines Count", is_mines)
        self._set_field_enabled("Safe Picks", is_mines)
        if is_dice_like:
            for field_name in ("Multiplier / Target", "Win Chance %"):
    ...
    def _share_stress_result(self, *args):
        ...
        share_result(f"Strategy Stress Test ({r['game'].upper()})", [
            f"Average Profit:        {r['avg']:.4f}",
            f"Median Profit:         {r['med']:.4f}",
            f"Best Session:          {r['best']:.4f}",
            f"Worst Session:         {r['worst']:.4f}",
            f"Profitable Sessions:   {r['wr']:.2f}%",
            f"Bust Rate:             {r['br']:.2f}%",
            f"Longest Loss Streak:   {r['ls']}",
        ])
...
class BankrollSurvivalScreen(Screen):
    def __init__(self, **kw):
        super().__init__(**kw)
        outer = BoxLayout(orientation='vertical', padding=dp(10), spacing=dp(6))
        outer.add_widget(build_info_header("BANKROLL SURVIVAL LAB"))
    ...
    def _share_survival_result(self, *args):
        ...
        if not r:
            return
        share_result("Bankroll Survival Lab", [
            f"Survival Chance:       {r['survival']:.2f}%",
            f"Bust Chance:           {r['bust']:.2f}%",
            f"Average Profit:        {r['avg']:.4f}",
...
class MainMenu(Screen):
    def __init__(self, **kw):
        ...
        self.bank_lbl = Label(text="PROFIT: $0.00 | TIME: 00:00", size_hint_y=None, height=dp(22), color=get_color_from_hex(STAKE_GREEN), bold=True)
        Clock.schedule_interval(self.update_header, 1)
        self.title_lbl = Label(text="Casino Tools Pro  [size=18]v6.3[/size]", markup=True, font_size='32sp', bold=True, size_hint_y=None, height=dp(40), color=get_color_from_hex(STAKE_GREEN), halign='center', valign='middle')
        self.subtitle_lbl = Label(text="Private gambling analytics toolkit", color=get_color_from_hex(SUBTITLE_TEXT), font_size='11sp', size_hint_y=None, height=dp(14))
        ...
        self.presentation_toggle_btn = StyledButton(text='PRESENTATION MODE: OFF', bg_color=PRESENTATION_ACCENT_ALT, height=dp(34))
        ...
        self.featured_grid = GridLayout(cols=3, spacing=dp(6), size_hint_y=None, height=dp(114))
        self.featured_defs = [
        ...
        for sid, color, tall in self.featured_defs:
            tile = self.make_tile(MENU_TILE_LABELS.get(sid, TOOL_TITLES.get(sid, sid)), sid, color, tall=tall)
        ...
        for section_key, items in self.section_configs:
            ...
            for sid, color in items:
                tile = self.make_tile(MENU_TILE_LABELS.get(sid, TOOL_TITLES.get(sid, sid)), sid, color, compact=True)
                self.tile_buttons[sid] = tile
        ...
        history_btn = StyledButton(text='HISTORY', bg_color=UTILITY_COLOR, size_hint_x=0.22, height=dp(40))
    ...
    def make_tile(self, title_text, sid, color, tall=False, compact=False):
        tile_h = dp(54) if tall else (dp(50) if compact else dp(56))
        font_size = '13sp' if tall else '12sp'
    ...
    def get_menu_tile_text(self, sid):
        if App.get_running_app() and App.get_running_app().is_presentation_mode_active():
            return PRESENTATION_MENU_TILE_LABELS.get(sid, PRESENTATION_TOOL_TITLES.get(sid, sid))
        return MENU_TILE_LABELS.get(sid, TOOL_TITLES.get(sid, sid))

    def refresh_presentation_mode_ui(self):
        app = App.get_running_app()
        active = bool(app and app.is_presentation_mode_active())
        available = bool(app and app.get_tier() == PRO_PLUS)

        self.presentation_row.height = dp(34) if available else dp(0)
        self.presentation_row.opacity = 1 if available else 0
        self.presentation_toggle_btn.disabled = not available
        self.presentation_toggle_btn.text = 'PRESENTATION MODE: ON' if active else 'PRESENTATION MODE: OFF'

        self.title_lbl.text = f"{get_display_app_name()}  [size=18]v6.3[/size]"
        ...
        self.p_in.hint_text = 'Enter Result (+/-)' if active else 'Enter Win/Loss (+/-)'

        self.mode_badge_lbl.height = dp(14) if active else dp(0)
        self.mode_badge_lbl.text = 'PRESENTATION MODE • DEMO WORDING' if active else ''
    ...
    def show_about(self, *args):
        if App.get_running_app() and App.get_running_app().is_presentation_mode_active():
            ...
            msg = (
                'Strategy Suite Pro is a probability, simulation, and scenario-analysis toolkit created for educational '
                'demonstrations, structured testing, and visual presentations. It is designed to help users study risk, '
                'variance, sequence behavior, and model performance in a cleaner and more understandable way.\n\n'
                'Presentation Mode only changes selected wording and visuals for polished showcase footage. It does not '
                'alter the underlying calculations, safety messaging, or licensing behavior.\n\n'
                'Important Disclaimer\n'
                '• This software is for analysis, education, and demonstration purposes only.\n'
                '• It does not promise profit, guaranteed outcomes, or financial success.\n'
                '• It should not be interpreted as betting advice, investment advice, or a prediction engine.\n'
                '• Users remain fully responsible for how they apply any information shown by the tool.\n\n'
                'Created by SH Vertex Tech Labs'
            )
        else:
            popup_title = 'About Casino Tools Pro'
            msg = (
                'Casino Tools Pro is an analytics and simulation toolkit built to help users study bankroll behavior, '
                'variance, progression logic, discipline, and betting structures more clearly. It was created to explore '
                'risk mathematically, not to glamorize gambling.\n\n'
                'Important Disclaimer\n'
        ...
        popup = Popup(title=popup_title, content=content, size_hint=(0.88, 0.70))
        close_btn.bind(on_release=lambda *a: popup.dismiss())
        content.add_widget(scroll)
    ...
    def show_history(self, *args):
        ...
        summary = Label(
            text=f"Total: {unit}{total:.2f}  |  Updates: {len(entries)}  |  W: {len(wins)}  L: {len(losses)}",
        ...
        if not entries:
            entry_list.add_widget(Label(
                text="No history yet.\nUse the UPDATE button on the main screen to log profit/loss entries.",
                color=get_color_from_hex(STAKE_TEXT),
                halign='center',
                valign='middle',
                size_hint_y=None,
    ...
    def reset_tracker(self, *args):
        content = BoxLayout(orientation='vertical', padding=dp(12), spacing=dp(10))
        content.add_widget(Label(text='Reset total profit and elapsed time tracker?', color=get_color_from_hex(STAKE_TEXT), size_hint_y=None, height=dp(40)))
    ...
    def update_header(self, dt):
        unit = get_display_unit_prefix()
        label = 'UNITS' if unit == 'U' else 'PROFIT'
        self.bank_lbl.text = f"{label}: {unit}{GLOBAL_BANK.session_profit:.2f} | TIME: {GLOBAL_BANK.get_duration()}"
        try:
...
class CasinoApp(App):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.presentation_mode_enabled = load_presentation_mode_state()
    ...
    def get_demo_status_line(self):
        ...
        if tier == PRO_PLUS:
            suffix = "  |  PRESENTATION MODE" if self.is_presentation_mode_active() else ""
            return f"PRO+ ACTIVE{'  |  ' + LICENSE_STATE.license_id if LICENSE_STATE.license_id else ''}{suffix}"
        ...
        return (
            ...
            f"MC {DEMO_USAGE.remaining('mc')}  |  BJ {DEMO_USAGE.remaining('bj')}  |  Saves {DEMO_USAGE.remaining('strats_save')}{msg}"
        )


    def is_presentation_mode_active(self):
        return self.get_tier() == PRO_PLUS and bool(self.presentation_mode_enabled)

    def set_presentation_mode(self, enabled):
        self.presentation_mode_enabled = bool(enabled) if self.get_tier() == PRO_PLUS else False
        save_presentation_mode_state(self.presentation_mode_enabled)
        try:
